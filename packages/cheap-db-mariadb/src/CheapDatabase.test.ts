/**
 * Unit tests for MariaDB CheapDatabase
 *
 * NOTE: These tests require a running MariaDB instance.
 ...
 * - MARIADB_DB (default: cheap_test)
 * - MARIADB_USER (default: root)
 * - MARIADB_PASSWORD (default: password)
 */

import { describe, it, expect, beforeEach, afterEach, beforeAll, afterAll } from "@jest/globals";
import { CheapDatabase, type MariaConfig } from "./CheapDatabase.js";
import { randomUUID } from "crypto";

// Check if MariaDB is configured for testing
const isMariaDBAvailable = process.env.MARIADB_TEST === "true";

const mariaConfig: MariaConfig = {
...
const describeIfMariaDB = isMariaDBAvailable ? describe : describe.skip;

describeIfMariaDB("MariaDB CheapDatabase", () => {
  let db: CheapDatabase;

  beforeAll(async () => {
    // Create a test database instance and initialize schema
    db = new CheapDatabase(mariaConfig);
    await db.initSchema();
  });
...
if (!isMariaDBAvailable) {
  describe.skip("MariaDB CheapDatabase (skipped)", () => {
    it("requires MARIADB_TEST=true environment variable", () => {
      console.log(`
        MariaDB tests are skipped. To run them:
        1. Start a MariaDB instance
        2. Create a test database (e.g., cheap_test)
        3. Set environment variables:
           export MARIADB_TEST=true
